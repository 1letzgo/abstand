...
struct SettingsHubRootView: View {
  ...
  /// Untertitel für „Manage prepare": wartende + fertige Session-Preps.
  private var managePrepareSubtitle: String? {
    let pending = model.readAlongPrepare.pendingCount
    let ready = model.readAlongPrepare.readyItemIds.count
    if pending == 0, ready == 0 { return nil }
    var parts: [String] = []
  ...
  private var managePrepareBadge: String? {
    let pending = model.readAlongPrepare.pendingCount
      ...
      case .downloads:
        ServerAdminSection(title: "Downloads") {
          LazyVStack(spacing: AppTheme.Layout.withinSectionSpacing) {
            ...
            .buttonStyle(.plain)
            NavigationLink {
              SettingsReadAlongPrepareManageView()
              ...
              AbstandGroupedCard {
                ServerAdminNavRow(
                  ...
                  title: "Manage prepare",
                  subtitle: managePrepareSubtitle,
                  badge: managePrepareBadge
                )
              }
            }
            .buttonStyle(.plain)
...
// MARK: - Manage read-along prepare queue

/// Zeigt aktive/wartende Prepare-Jobs sowie in dieser Session fertige Items.
private struct SettingsReadAlongPrepareManageView: View {
  @EnvironmentObject private var model: AppModel

  var body: some View {
    SettingsReadAlongPrepareManageContent(queue: model.readAlongPrepare)
      .themeAccentFromAppModel(model)
      .abstandThemeRefresh()
      .toolbarTitleDisplayMode(.inlineLarge)
      .tint(model.appearanceAccentColor)
...
private struct SettingsReadAlongPrepareManageContent: View {
  @EnvironmentObject private var model: AppModel
  @Environment(\.themeAccent) private var themeAccent
  @ObservedObject var queue: ReadAlongPrepareQueue
  @State private var activeProgress: Double = 0
  ...
  var body: some View {
    ServerAdminScrollScreen {
      LazyVStack(alignment: .leading, spacing: AppTheme.Layout.sectionSpacing) {
        activeSection
        readySection
        if let error = queue.lastErrorMessage, queue.pendingCount == 0 {
          ServerAdminSection(title: "Last error") {
            AbstandGroupedCard {
              Text(error)
                .font(.subheadline)
                .foregroundStyle(.red)
    ...
    .onReceive(queue.$progress) { p in
      guard queue.activeItemId != nil else { return }
      activeProgress = p
    }
    .onReceive(queue.$activeItemId) { _ in
      activeProgress = queue.progress
    }
    .onAppear { activeProgress = queue.progress }
  }

  @ViewBuilder
  private var activeSection: some View {
    let activeId = queue.activeItemId
    let queued = queue.queuedItemIds
    if activeId != nil || !queued.isEmpty {
      ServerAdminSection(title: "In progress") {
        LazyVStack(spacing: AppTheme.Layout.withinSectionSpacing) {
          if let activeId {
            let entry = queue.catalogById[activeId]
            AbstandGroupedCard {
              ReadAlongPrepareManageRow(
                entry: entry,
                ...
                onCancel: { model.cancelReadAlongPrepare(itemId: activeId) }
              )
            }
          }
          ForEach(queued, id: \.self) { qid in
            let entry = queue.catalogById[qid]
            AbstandGroupedCard {
              ReadAlongPrepareManageRow(
                entry: entry,
                progress: 0,
                ...
                cacheAccount: model.coverImageCacheAccountDirectory(),
                cacheRevision: model.coverImageCacheRevision,
                onCancel: { model.cancelReadAlongPrepare(itemId: qid) }
              )
            }
          }
        }
  ...
  private var readySection: some View {
    ServerAdminSection(title: "Ready") {
      if queue.readyItemIds.isEmpty {
        AbstandGroupedCard {
          VStack(alignment: .leading, spacing: 4) {
            Text("No prepared titles yet.")
              .font(.body)
              .foregroundStyle(model.appearancePalette.textPrimary)
            Text("Prepare audiobooks or episodes from their detail screen. Progress appears here.")
              .font(.caption)
              .foregroundStyle(model.appearancePalette.textSecondary)
          }
          .frame(maxWidth: .infinity, alignment: .leading)
        ...
        LazyVStack(spacing: AppTheme.Layout.withinSectionSpacing) {
          ForEach(queue.readyItemIds, id: \.self) { rid in
            let entry = queue.catalogById[rid]
            AbstandGroupedCard {
              ReadAlongPrepareManageRow(
                entry: entry,
                progress: 1,
...
/// Zeile für aktiven, wartenden oder fertigen Prepare-Job.
private struct ReadAlongPrepareManageRow: View {
  @EnvironmentObject private var model: AppModel
  @Environment(\.themeAccent) private var themeAccent
  ...
  private var titleText: String {
    entry?.title.nilIfEmpty
      ?? (isQueued
        ? String(localized: "Waiting…", comment: "Read along prepare queued placeholder")
  ...
  var body: some View {
    HStack(spacing: 12) {
      if let lid = entry?.libraryItemId {
        ...
        SettingsCardIcon(
          systemName: isQueued ? "circle.dashed" : (isReady ? "checkmark.circle" : "text.book.closed"),
          tint: themeAccent
        )
      }
      ...
      .accessibilityLabel(
        isReady
          ? "Dismiss ready item"
          : (isQueued ? "Remove from queue" : "Cancel prepare")
      )
    }
  }
}
