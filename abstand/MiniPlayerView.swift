...
private struct SleepTimerUtilityMenuLabel: View {
  ...
  private var sleepAccessibilityLabel: String {
    if let seconds = player.sleepTimerDisplaySeconds, seconds > 0 {
      return String(
        localized: "Sleep timer, \(formatPlaybackTime(max(0, seconds))) remaining",
        comment: "Accessibility"
      )
    }
    if case .chapters(let n) = player.sleepTimerMode, n > 0 {
      return String(
        localized: "Sleep timer, \(n) chapter(s)",
        comment: "Accessibility"
      )
    }
    return String(localized: "Sleep timer, Off", comment: "Accessibility")
  }
...
private struct SleepTimerCapsuleStepper: View {
  ...
  private func stepperButton(systemName: String, enabled: Bool, action: @escaping () -> Void) -> some View {
    ...
    .accessibilityLabel(
      systemName.contains("minus")
        ? String(localized: "Decrease sleep timer", comment: "Accessibility")
        : String(localized: "Increase sleep timer", comment: "Accessibility")
    )
  }
}
...
private struct FullPlayerUtilityBar: View, Equatable {
  ...
  var body: some View {
    ...
    return HStack(spacing: 0) {
      ...
      .accessibilityLabel(
        String(localized: "Equalizer, \(eqPreset.label)", comment: "Accessibility")
      )
      ...
      .accessibilityLabel(
        String(
          localized: "Playback speed, \(miniPlayerFormatPlaybackRate(playbackRate))",
          comment: "Accessibility"
        )
      )
      ...
      VStack(spacing: FullPlayerUtilityBarLayout.rowSpacing) {
        FullPlayerAirPlayButton()
          ...
          .accessibilityLabel(String(localized: "AirPlay", comment: "Accessibility"))
  ...
  return HStack(alignment: .center, spacing: 0) {
      ...
      .accessibilityLabel(
        isBusy
          ...
          : (snapshot.isPlaying
            ? String(localized: "Pause", comment: "Accessibility")
            : String(localized: "Play", comment: "Accessibility"))
      )
      .frame(maxWidth: .infinity)
...
struct NowPlayingDetailView: View {
  ...
  private var fullPlayerStack: some View {
    ...
    GeometryReader { geo in
      ...
      .animation(
        reduceMotion ? nil : .interactiveSpring(response: 0.3, dampingFraction: 0.86),
  ...
  private func fullPlayerPortraitLayout(book: ABSBook) -> some View {
    ...
    .animation(reduceMotion ? nil : .easeInOut(duration: 0.28), value: panelExpanded)
    .animation(reduceMotion ? nil : .easeInOut(duration: 0.28), value: coverPanel)
  }
  ...
  private func fullPlayerExpandedListCard(book: ABSBook) -> some View {
    ...
    .animation(reduceMotion ? nil : .easeInOut(duration: 0.22), value: coverPanel)
  }
